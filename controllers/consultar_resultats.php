    ...
    if($_SESSION['permiso_defecto'] == "ninguno" && $_SESSION['permiso_ambito'] == "ninguno") {
        ...
        if (isset($_GET['as'])) {
            ...
            if ($_SESSION['permiso_defecto'] == "ninguno" && $_SESSION['permiso_ambito'] != "ninguno"
                ...
                $llistat_respostes10 = obert_tot_profes(connection(), "AssigG10",$_SESSION['niu'] ,$pla,
                                                                "$edicio", "$assignatura");
                $llistat_respostes11 = obert_tot_profes(connection(), "AssigG11", $_SESSION['niu'] ,$pla,
                                                                    "$edicio", "$assignatura");

                $participacio = participacio_profe(connection(), "$edicio", $_SESSION['niu'],"$pla", "$assignatura");
                $matriculats = matriculats_segons_profe(connection(), $anio_edicio[0]['anio_inicio'], "$edicio", "$pla", $_SESSION['niu'], "$assignatura");
                $llista_grups = grup_profes(connection(), "$edicio",$_SESSION['niu'], "$pla", "$assignatura");
               ...
               /* if (sizeof($llista_grups) > 1) {
                ...
                /*if ($_SESSION['grup'] != NULL) {
                    $participacio = participacio_grup_profes(connection(), "$edicio", "$assignatura", "$grup");
                    $matriculats = matriculats_grup_profes(connection(), "$assignatura", $anio_edicio[0]['anio_inicio'], "$grup");
                }*/
            }else{
